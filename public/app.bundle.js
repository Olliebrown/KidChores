        ...
        switch (_context.prev = _context.next) {
          case 0:
            return _context.abrupt("return", new Promise(function (resolve, reject) {
              ...
              jquery__WEBPACK_IMPORTED_MODULE_0___default.a.get(AJAXSettings).fail(function (jqXHR, textStatus) {
                if (jqXHR.status === 401 || jqXHR.status === 403) {
                  resolve({
                    unauthorized: true
                  });
                } else {
                  reject(new Error("Failed to retrieve task data (".concat(jqXHR.status, ")")));
                }
              });
            }));
...
var DEFAULT_COLLAPSE = null;
var userInfo = null; // Runs when the document is fully loaded and the DOM is ready
...
jquery__WEBPACK_IMPORTED_MODULE_3___default()(document).ready(function () {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_3___default()('#logoutLink').click(function (event) {
    event.preventDefault();
    Object(_user__WEBPACK_IMPORTED_MODULE_6__["logoutUser"])();
    userInfo = undefined;
    Object(_user__WEBPACK_IMPORTED_MODULE_6__["updateUserState"])(userInfo);
    Object(_data__WEBPACK_IMPORTED_MODULE_5__["retrieveCategorySchema"])(processTaskData);
  });
...
function loginSubmit(event) {
  event.preventDefault();
  Object(_user__WEBPACK_IMPORTED_MODULE_6__["loginExistingUser"])().then(function (loginInfo) {
    if (loginInfo) {
      userInfo = loginInfo;
      Object(_data__WEBPACK_IMPORTED_MODULE_5__["retrieveCategorySchema"])(processTaskData);
    }
...
function processTaskData(data) {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_3___default()('#category-list').empty(); // Check if we were properly authorized

  if (data.unauthorized) {
    var loginMessage = jquery__WEBPACK_IMPORTED_MODULE_3___default()('<h5/>').addClass('text-center m-3').text('Please login (click login in the nav bar)');
    jquery__WEBPACK_IMPORTED_MODULE_3___default()('#category-list').append(loginMessage);
  } else {
    // Create the task matrix accordion and append it to our main content area
    var taskMatrix = buildTaskMatrix(data);

    if (taskMatrix != null) {
      jquery__WEBPACK_IMPORTED_MODULE_3___default()('#category-list').append(taskMatrix);
...
function buildTaskMatrix(data) {
  ...
  var accordionTag = jquery__WEBPACK_IMPORTED_MODULE_3___default()('<div/>').addClass('accordion').attr('id', ACCORDION_ID); // Keep track of which one should be open by default

  var currentHour = Object(_utils__WEBPACK_IMPORTED_MODULE_4__["currentHourLocal"])();
  DEFAULT_COLLAPSE = null;
  var defaultCollapseHour = 0; // Create each collapseable card for the categories and add to accordion

  data.categories.forEach(function (category) {
    var categoryCard = buildCategory(category);
...
function toggleDefaultCollapse() {
  ...
  if (DEFAULT_COLLAPSE) {
    var collapseID = "#collapse-cat".concat(DEFAULT_COLLAPSE);
    jquery__WEBPACK_IMPORTED_MODULE_3___default()(collapseID).collapse({
      toggle: true
    });
  }
}
...
function buildCategory(categoryData) {
  ...
  */
  ...
  var categoryID = "cat".concat(categoryData.id);
  var headerID = "heading-".concat(categoryID);
  var collapseID = "collapse-".concat(categoryID); // Build the category card header which will trigger the collapse

  var cardHeaderTag = jquery__WEBPACK_IMPORTED_MODULE_3___default()('<div/>').addClass('cardHeader').attr('id', headerID);
  var headerTextTag = jquery__WEBPACK_IMPORTED_MODULE_3___default()('<h4/>').addClass('card-title m-3').attr('data-toggle', 'collapse').attr('data-target', "#".concat(collapseID)).attr('aria-expanded', false).attr('aria-controls', collapseID).text(categoryData.name);
  cardHeaderTag.append(headerTextTag); // Build the category card collapse which holds the body of the category card

  var cardCollapseTag = jquery__WEBPACK_IMPORTED_MODULE_3___default()('<div/>').addClass('collapse').attr('id', collapseID).attr('aria-labelledby', headerID).attr('data-parent', "#".concat(ACCORDION_ID));
  var cardBody = jquery__WEBPACK_IMPORTED_MODULE_3___default()('<div/>').addClass('card-body');
  cardCollapseTag.append(cardBody); // Build the inner tasks cards stored in a card column container

  var tasksCardsTag = jquery__WEBPACK_IMPORTED_MODULE_3___default()('<div/>').addClass('card-columns');
  categoryData.tasks.forEach(function (task) {
    var cardTag = buildCard(task, categoryData.id);
  ...
  cardBody.append(tasksCardsTag); // Put the category card all together

  var categoryCardTag = jquery__WEBPACK_IMPORTED_MODULE_3___default()('<div/>').addClass('card categoryCard').attr('id', categoryID);
  categoryCardTag.append(cardHeaderTag);
  categoryCardTag.append(cardCollapseTag); // Return the category card
...
function buildCard(cardData, catID) {
  ...
  var taskID = "task".concat(cardData.id); // Make the header with a nice looking title

  var cardHeaderTag = jquery__WEBPACK_IMPORTED_MODULE_3___default()('<div/>').addClass('card-header');
  cardHeaderTag.append(jquery__WEBPACK_IMPORTED_MODULE_3___default()('<h5/>').addClass('card-title').text(cardData.name)); // Make the body of the card with centered text

  var cardBodyTag = jquery__WEBPACK_IMPORTED_MODULE_3___default()('<div/>').addClass('card-body text-center');
  cardBodyTag.append(jquery__WEBPACK_IMPORTED_MODULE_3___default()('<p/>').addClass('card-text').text(cardData.details)); // Make a footer with a couple of butons (we'll hide one later)

  var cardFooterTag = jquery__WEBPACK_IMPORTED_MODULE_3___default()('<div/>').addClass('card-footer');
  cardFooterTag.append(jquery__WEBPACK_IMPORTED_MODULE_3___default()('<a/>').addClass('btn btn-success').text('Did It!'));
  cardFooterTag.append(jquery__WEBPACK_IMPORTED_MODULE_3___default()('<a/>').addClass('btn btn-danger').text('Nope!')); // Put the final card together and return it
  ...
  var cardTag = jquery__WEBPACK_IMPORTED_MODULE_3___default()('<div/>').addClass('card border rounded taskCard').attr('id', taskID);
...
function checkToken() {
  return new Promise(function (resolve, reject) {
    var token = js_cookie__WEBPACK_IMPORTED_MODULE_1___default.a.get(_config__WEBPACK_IMPORTED_MODULE_2__["default"].cookieName);

    if (!token) {
      reject(new Error('no token'));
    } else {
      var AJAXSettings = Object(_utils__WEBPACK_IMPORTED_MODULE_3__["makeAJAXSettings"])("/data/authorize/", function (result) {
        resolve(result);
      }, {
        token: token
...
function validateUser(username, password) {
  return new Promise(function (resolve, reject) {
    ...
    if (false) {} // Build the POST request settings
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            username = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#inputNewUserName').val();
            _context.next = 4;
      ...
      while (1) {
        switch (_context3.prev = _context3.next) {
          ...
          case 5:
            ...
            if (result.token) {
              js_cookie__WEBPACK_IMPORTED_MODULE_1___default.a.set(_config__WEBPACK_IMPORTED_MODULE_2__["default"].cookieName, result.token, {
                expires: 7
              });
...
function logoutUser() {
  ...
  js_cookie__WEBPACK_IMPORTED_MODULE_1___default.a.remove(_config__WEBPACK_IMPORTED_MODULE_2__["default"].cookieName);
}
function updateUserState(userInfo) {
  console.log('updating user info');

  if (userInfo) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#navLogin').attr('hidden', '');
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#navUsername').removeAttr('hidden');
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#navUsernameText').html("Hello ".concat(userInfo.firstname, " &nbsp;&nbsp;|"));
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#navLogout').removeAttr('hidden');
...
function makeAJAXSettings(URL, successCallback, payloadData) {
  var responseDataType = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'json';
  // Build basic settings object
  var settings = {
    url: URL,
    success: successCallback,
    dataType: responseDataType // Add data if provided
  ...
  var JWT = js_cookie__WEBPACK_IMPORTED_MODULE_0___default.a.get(_config__WEBPACK_IMPORTED_MODULE_1__["default"].cookieName);
...
module.exports = __webpack_require__(/*! /Users/berriers/Documents/KidsChores/src/main.js */"./src/main.js");
